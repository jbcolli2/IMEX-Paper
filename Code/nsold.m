function [sol, it_hist, ierr, x_hist] = nsold(x,f,tol,parms,varargin)
...
%
    ...
    if jdiff == 1 
            if iband == 0
            [l, u] = diffjac(x,f,f0,varargin{:});
            else
            jacb = bandjac(f,x,f0,nl,nu); 
            [l,u] = lu(jacb);
            end
        else
            [fv,jac] = feval(f,x,varargin{:});
        [l,u] = lu(jac);
        end
    end
    itsham = itsham-1;
...
%
    jac_age = jac_age+1;
    xold = x; fold = f0;
    [step,iarm,x,f0,armflag] = armijo(direction,x,f0,f,maxarm,varargin{:});
%
...
%
%
function [l, u] = diffjac(x, f, f0,varargin)
...
for j = 1:n
    zz = zeros(n,1);
    zz(j) = 1;
    jac(:,j) = dirder(x,zz,f,f0, varargin{:});
end
[l, u] = lu(jac);
...
%
%
function z = dirder(x,w,f,f0,varargin)
...
%
del = x+epsnew*w;
f1 = feval(f,del,varargin{:});
z = (f1 - f0)/epsnew;
%
% Compute the step length with the three point parabolic model.
%
function [step,iarm,xp,fp,armflag] = armijo(direction,x,f0,f,maxarm,varargin)
iarm = 0;
sigma1 = .5;
alpha = 1.d-4;
armflag = 0;
